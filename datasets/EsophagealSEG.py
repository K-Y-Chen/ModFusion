...
class huaxi_dataset(Dataset):
    def __init__(self, base_dir, num_classes=4, split='train', transform=None):
        ...
        for m in self.modality:
            with open(os.path.join(base_dir, m, f"ImageSets/{split}.txt"), "r") as f:
                names = f.readlines()
                # print(len(names))
                for img_name in names:
                    self.image_list.append(os.path.join(base_dir, m, 'images', img_name.replace('\n', '') + '_image.jpg'))   
                    self.label_list.append(os.path.join(base_dir, m, 'labels', img_name.replace('\n', '') + '_label.png')) 
        # self.image_list = self.image_list[:200]
        # self.label_list = self.label_list[:200]
    ...
    def __getitem__(self, idx):
        ...
        # print(image_path)
        image_name = os.path.basename(image_path)[:-10]
        ...
        m = image_path.split(r'/')[-3]
        cls_label = self.m2id[m]
        cls_label = torch.tensor(cls_label).long()
        cls_label = F.one_hot(cls_label, len(self.m2id)).float()
