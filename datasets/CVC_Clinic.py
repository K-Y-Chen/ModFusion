...
class CVC_Clinic(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label[label >= self.num_classes] = 1
        # print(image.shape)
...
if __name__ == '__main__':
    ...
    args.dataset = 'CVC_Clinic'
    args.base_dir = '/workspace/huaxicolor/Datasets/CVC-ClinicDB'
    
    train = CVC_Clinic(args.base_dir, num_classes=2, split='train', transform=transforms.Compose([RandomGenerator(output_size=[224, 224])]))
    print(len(train))
    ...
    for ii, sample in enumerate(dataloader):
        for jj in range(sample["image"].size()[0]):
            ...
            # print(img_tmp.shape, segmap.shape)
            plt.figure()
            plt.title('display')
            plt.subplot(211)
            plt.imshow(img_tmp)
