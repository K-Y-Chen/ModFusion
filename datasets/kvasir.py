...
class KvasirSEG(Dataset):
    def __init__(self, base_dir, num_classes=2, split='train', transform=None):
        ...
        for idx in range(len(images)):
            ...
            self.image_list.append(os.path.join(base_dir, 'images', os.path.basename(images[idx]).replace('png', 'jpg')))
            self.label_list.append(os.path.join(base_dir, 'masks', os.path.basename(images[idx]).replace('png', 'jpg')))
...
if __name__ == '__main__':
    ...
    args.dataset = 'Kvasir'
    args.base_dir = '/workspace/huaxicolor/Datasets/KvasirSEG'
    
    train = KvasirSEG(args.base_dir, num_classes=2, split='train', transform=transforms.Compose([RandomGenerator(output_size=[224, 224])]))
    print(len(train))
